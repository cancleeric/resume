...
## v8.81 · 2026-05-26
- seo(C版+dict): index-light.html + dict-zh/en og.description 補「SaaS 最快 23 天上線」proof point——og:description 社群分享卡片三版一致，提升 LinkedIn/Twitter 點擊率
...
## v8.35 · 2026-05-26
- proof(C版): hero.claim 移除「體感約 20×」自評語（ZH/EN 同步）；補「214 commits，git 紀錄可查」——與 A版 R30 / B版 R28 全面對齊，三版「20×」清零

## v8.34 · 2026-05-26
- proof(B版): agency.row.06「驗證程度」欄強化——「logo wall，實際參與深度不一」改為「無法核實執行工程師與技術深度」，對比 studio git 可查指標更具體（ZH/EN 同步，C版同受益）
...
## v8.27 · 2026-05-26
- wcag(C版): 系統狀態標籤 LIVE/GA/BETA 亮底對比不足（#16A34A ~3.25:1 / #0EA5E9 ~2.74:1 / #D97706 ~3.19:1）；C版 override 至 #166534 / #0369A1 / #92400E 全達 AA

## v8.26 · 2026-05-26
- trust(B/C版): faq.03 定價 FAQ 重構——原文「超工」框架暗示加班；改為「產出密度」框架，補入 SaaS 23 天實證數據 + spec→ops 全覆蓋對比 hourly 單層交付（ZH/EN 同步）

## v8.25 · 2026-05-26
- trust(A版): case-firebase 補充模組涵蓋範圍——原文只列工時與模組數量，缺乏業務情境；補入「玩家管理、行為分析、客服工單、廣告收益監控」讓 656+ 工時有具體交付內容支撐（ZH/EN 同步）
...
- loop-opt(標準版)：footer 補統編 80289779 + 版號；sidebar__foot 與 bottom em 補版號 v7.97，三處同步，信任要件對齊設計版。

---

## v7.89 — 2026-05-23

- loop-opt(標準版)：專案制 (US$8K 起) 報價卡 CTA 改帶意圖直連 mailto。原 CTA「討論專案範圍」只連 `#預約諮詢` 錨點，與已升級的健檢（v7.74）、月費 Retainer（v7.80）兩張卡有明顯落差。補上主旨預填【專案制諮詢】+ 5 欄正文（範圍／時程／預算／公司／聯絡時段），CTA 文案 zh+en 對齊「主旨已預填，2 分鐘寄出」風格。三張高意圖卡 CTA 機制全部對齊；顧問諮詢卡（低票價輔助商品）暫不動。
...
- loop-opt(設計版·深)：設計版 SEO 標題去除未驗證主張。深/亮兩版的 `<title>` 與 `twitter:title` 仍寫「20× 產出」當硬主張——但 v7.73 已把「20×」全站降級為明標「體感約 20×」，body/dict 都已修，只剩 SEO meta 沒跟上。改為「一年交付 14+ 產品」（可查證、與標準版 title 一致）。深、亮兩版各 2 處。

---

## v7.80 — 2026-05-22

- loop-opt(標準版)：月費 Retainer 報價卡 CTA 降摩擦。原「主力推薦」卡 CTA 只丟通用 `#預約諮詢` 錨點，反而比「2 週健檢」低風險入口卡（v7.74 已帶意圖直連 mailto）摩擦更高——最想成交的方案路徑最長。改為帶意圖直連 mailto，主旨預填「【預約月費 Retainer】」、正文預填 4 欄（團隊簡述／想自動化環節／合作起始時間／聯絡時段），CTA 文案 zh+en 對齊健檢卡「主旨已預填，2 分鐘寄出」。
